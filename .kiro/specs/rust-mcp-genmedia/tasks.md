...
- [x] 1. Initialize Cargo workspace and project structure
...
- [x] 2. Implement adk-rust-mcp-common configuration module
  - [x] 2.1 Create Config struct and ConfigError types
  ...
  - [x] 2.2 Write property test for config defaults
    - **Property 1: Configuration Loading with Defaults**
    - **Validates: Requirements 2.1, 2.3, 2.4, 2.5**

- [x] 3. Implement adk-rust-mcp-common GCS module
  - [x] 3.1 Create GcsUri struct and parsing
  ...
  - [x] 3.2 Write property test for GCS URI round-trip
    - **Property 2: GCS URI Round-Trip Parsing**
    - **Validates: Requirements 2.9**
  - [x] 3.3 Implement GcsClient for upload/download
  ...
  - [x] 3.4 Write unit tests for GCS operations with mocked API
    - Test upload success and failure scenarios
    - Test download success and failure scenarios
    - _Requirements: 2.7, 2.8, 2.10_

- [x] 4. Implement adk-rust-mcp-common models module
  - [x] 4.1 Define model structs and registry
  ...
  - [x] 4.2 Write property test for model alias resolution
    - **Property 4: Model Alias Resolution Consistency**
    - **Validates: Requirements 2.14**

- [x] 5. Implement adk-rust-mcp-common auth module
  - [x] 5.1 Create AuthProvider using ADC
  ...
  - [x] 5.2 Write property test for token refresh
    - **Property 18: Token Auto-Refresh**
    - **Validates: Requirements 11.7**

- [x] 6. Implement adk-rust-mcp-common error types
  - [x] 6.1 Create unified Error enum
  ...
  - [x] 6.2 Write property test for error context
    - **Property 16: Error Context Inclusion**
    - **Validates: Requirements 10.5, 10.6**
  - [x] 6.3 Initialize tracing with env_logger
...
- [x] 7. Checkpoint - Verify adk-rust-mcp-common builds and tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [x] 8. Implement MCP server framework utilities
  - [x] 8.1 Create transport configuration and server builder
  ...
  - [x] 8.2 Write unit tests for transport configuration
    - Test stdio default, HTTP port binding, SSE endpoint
    - _Requirements: 3.2, 3.3, 3.4, 3.5, 3.6_

- [x] 9. Implement adk-rust-mcp-image server
  - [x] 9.1 Create ImageHandler and parameter types
  ...
  - [x] 9.4 Implement image resources
  ...
  - [x] 9.5 Wire up adk-rust-mcp-image main.rs
  ...
  - [x] 9.6 Write unit tests for image handler
    - Test successful generation with mocked API
    - Test error handling for API failures
    - _Requirements: 4.15_

- [x] 10. Checkpoint - Verify adk-rust-mcp-image builds and tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [x] 11. Implement adk-rust-mcp-video server
  - [x] 11.1 Create VideoHandler and parameter types
  ...
  - [x] 11.2 Write property tests for video parameter validation
    - **Property 8: Numeric Parameter Range Validation** (duration_seconds)
    - **Property 9: Default Parameter Application**
    - **Validates: Requirements 5.4, 5.6**
  - [x] 11.3 Implement video_generate and video_from_image tools
  ...
  - [x] 11.4 Write property test for LRO polling
    - **Property 11: Long-Running Operation Polling**
    - **Validates: Requirements 5.16**
  - [x] 11.5 Wire up adk-rust-mcp-video main.rs
    - Register video_generate and video_from_image tools
    - Set up server with transport selection
    - _Requirements: 3.7, 3.9, 3.11, 3.12_
  - [x] 11.6 Write unit tests for video handler
    - Test LRO polling with mocked responses
    - Test error handling
    - _Requirements: 5.15, 5.16_

- [x] 12. Implement adk-rust-mcp-music server
  - [x] 12.1 Create MusicHandler and parameter types
    - Define MusicGenerateParams with validation
    - Implement sample_count range validation (1-4)
    - _Requirements: 6.2, 6.3, 6.4, 6.5, 6.6, 6.7_
  - [x] 12.2 Write property test for sample_count validation
    - **Property 8: Numeric Parameter Range Validation** (sample_count)
    - **Validates: Requirements 6.5**
  - [x] 12.3 Implement music_generate tool
    - Build Vertex AI Lyria API request
    - Handle WAV output (base64, local file, GCS)
    - _Requirements: 6.1, 6.8, 6.9, 6.10, 6.11_
  - [x] 12.4 Wire up adk-rust-mcp-music main.rs
    - Register music_generate tool
    - Set up server with transport selection
    - _Requirements: 3.7, 3.9, 3.11, 3.12_
  - [x] 12.5 Write unit tests for music handler
    - Test successful generation with mocked API
    - Test error handling
    - _Requirements: 6.11_

- [x] 13. Checkpoint - Verify adk-rust-mcp-video and adk-rust-mcp-music build and tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [x] 14. Implement adk-rust-mcp-speech server
  - [x] 14.1 Create SpeechHandler and parameter types
  ...
  - [x] 14.2 Write property tests for speech parameter validation
  ...
  - [x] 14.3 Implement speech_synthesize tool
...
- [x] 17. Implement adk-rust-mcp-avtool server
  - [x] 17.1 Create AVToolHandler and parameter types
  ...
  - [x] 17.8 Implement video processing tools
  ...
  - [x] 17.9 Wire up adk-rust-mcp-avtool main.rs
    - Register all 8 FFmpeg tools
    - Set up server with transport selection
    - _Requirements: 3.7, 3.9, 3.11, 3.12_
  - [x] 17.10 Write unit tests for AVTool handler
    - Test media info extraction
    - Test FFmpeg error handling
    - _Requirements: 9.19, 9.20_

- [x] 18. Checkpoint - Verify adk-rust-mcp-avtool builds and tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [x] 19. Implement OpenTelemetry tracing (optional)
  - [x] 19.1 Create otel module in adk-rust-mcp-common
  ...
  - [x] 19.2 Write unit tests for otel initialization
    - Test tracing setup when enabled
    - Test graceful handling when disabled
    - _Requirements: 2.15, 2.16_

- [x] 20. Final integration and validation
  - [x] 20.1 Add workspace-level integration tests
    - Test each server starts correctly with stdio transport
    - Test tool registration and schema generation
    - _Requirements: 3.7, 3.8_
  - [x] 20.2 Write property test for tool schema validity
    - **Property 5: Tool Schema Validity**
    - **Validates: Requirements 3.7, 3.8**
  - [x] 20.3 Write property test for input validation
    - **Property 6: Input Parameter Validation**
    - **Validates: Requirements 3.9**
  - [x] 20.4 Write property test for output format
    - **Property 7: Successful Tool Output Format**
    - **Validates: Requirements 3.11**
  - [x] 20.5 Update README.md with usage instructions
    - Update server status table to reflect completed implementations
